...
function sendConsolidatedAwardEmails(winningBids) {
    try {
        const notifications = {};
        winningBids.forEach(bid => {
            if (bid.VendorEmail && bid.RequisitionID) {
                const key = `${bid.VendorEmail}_${bid.RequisitionID}`;
                if (!notifications[key]) {
                    notifications[key] = {
                        vendorName: bid.VendorName, vendorEmail: bid.VendorEmail,
                        reqId: bid.RequisitionID, items: []
                    };
                }
                notifications[key].items.push({ name: bid.ItemName, amount: formatCurrency(bid.BidAmount) });
            }
        });

        for (const key in notifications) {
            const notification = notifications[key];
            const subject = `Contract Award Notification for Requisition ID: ${notification.reqId.split('-')[1]}`;
            ...
            const htmlBody = `
                <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
                    <div style="max-width: 600px; margin: auto; border: 1px solid #ddd; border-radius: 8px; padding: 20px;">
                        ...
                        <p>Congratulations! You have been awarded the following item(s) from <strong>Requisition ID ${notification.reqId.split('-')[1]}</strong>:</p>
            ...
            const plainBody = `Dear ${notification.vendorName},\n\nCongratulations! You have been awarded the following item(s) from Requisition ID ${notification.reqId.split('-')[1]}:\n\n${notification.items.map(i => `  • ${i.name}: ${i.amount}`).join('\n')}\n\nBest regards,\nDEB'S PROCUREMENT Team`;
